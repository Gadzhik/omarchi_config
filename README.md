...
| `hypr/bindings-personal.conf` | **Not in this repo** — bindings for specific apps and web apps (which mail provider, which messengers) live here, since that describes the owner rather than the machine. `bindings.conf` sources it, Hyprland fails the whole config if a sourced file is missing, so `install.sh` seeds it from `bindings-personal.conf.example` |
